 ...
 *
 * FastCGI/PHP-FPM hosting strips the Authorization header by default, which silently breaks
 * every Bearer-authenticated request (token AND OAuth) with an opaque auth challenge. The
 * automatic server setup (htaccess_fixer) and readiness pages call this endpoint server-side
 * with a dummy Authorization header to detect exactly that.
 *
